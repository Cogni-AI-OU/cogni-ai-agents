...
1. **Store Discovery**: Locate the canonical fact store based on the operational mode:
   ...
   - **Agent-Specific Mode**: For facts regarding agent behavior, rules, or invariants for the given project, the target is `AGENTS.md`, and `AGENTS.mmd` (if they exist, containing supplemental project diagrams, flows, and the booting sequence).
