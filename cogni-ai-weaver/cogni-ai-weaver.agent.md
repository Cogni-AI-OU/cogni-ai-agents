...
1. **Store Discovery**: Locate the canonical flow store based on the operational mode:
   ...
   - **Agent-Specific Mode**: For flowcharts detailing agent behavior, agent decision-making logic, or execution paths for the given project, the target is `AGENTS.md`, `FLOWS.mmd` and `AGENTS.mmd` (if they exist).
