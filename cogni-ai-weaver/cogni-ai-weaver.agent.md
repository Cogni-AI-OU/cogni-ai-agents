...
1. **Store Discovery**: Locate the canonical flow store based on the operational mode:
   - **Project-Specific Mode**: For project timelines, system state transitions, dependencies, or causal flows, the path is `../docs/AGENTS.mmd`.
   - **Agent-Specific Mode**: For flowcharts detailing agent behavior, agent decision-making logic, or execution paths for the given project, the target is `AGENTS.md` and `../docs/AGENTS.mmd` (if it exists).
