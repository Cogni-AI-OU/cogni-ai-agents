...
- **Global Protocol Subgraphs**: Does the diagram explicitly include or reference subgraphs from \`docs/FLOWS.mmd\`?
...
1. **Validation Report**: Detail any contradictions or missing constraints between the \``agent-name`.agent.md\` and the
   modeled states. List any rules from \`AGENTS.md\`/\`docs/FLOWS.mmd\` omitted.
